...
new = r'''service_details = {
    'free-websites-wilmington-nc': {
        ...
        'bullets': ['No upfront design bill for qualified service businesses', 'A clean, mobile-first website that makes your business easier to trust', 'Core service pages, local structure, FAQs, schema, and conversion paths built in from the start'],
        ...
        'process': ['Map the offer, service area, and best-fit customer before design starts', 'Start the site around trust, calls, texts, and AI-readable clarity', 'Launch with a structure that can support SEO, GBP, ads, and future service pages', 'Use the six-month growth partnership to keep expanding visibility instead of stopping at launch'],
        'outcome': 'You get a stronger online base layer first, then monthly marketing turns that base into a growing visibility system.'
    },
    'web-design-wilmington-nc': {
        'focus': 'Your website should make your business easier to trust before someone calls.',
        ...
        'process': ['Clarify the offer and the primary services people search for', 'Structure pages around real buyer decisions, not filler copy', 'Make the mobile experience quick, readable, and CTA-forward', 'Add schema, FAQs, and internal links so the site can support search and AI visibility'],
    ...
    'seo-ai-ranking-wilmington-nc': {
        ...
        'value': 'AI search does not replace local SEO. It raises the bar for clarity. Search engines and answer systems need to understand what you do, where you work, who you help, and why customers can trust you. That means the website, GBP, citations, FAQs, and service content need to reinforce each other.',
        'process': ['Strengthen the local website structure and service-area signals', 'Tune Google Business Profile categories, services, posts, and conversion paths', 'Build useful FAQ and service content that answers real buying questions', 'Keep citations and business details consistent so the business is easier to trust'],
    ...
    'google-ads-wilmington-nc': {
        ...
        'process': ['Choose the service searches most likely to produce real leads', 'Send visitors to pages that match the exact intent of the search', 'Track calls, texts, form inquiries, and which leads are worth more follow-up', 'Use results to adjust the page, offer, and campaign instead of only changing bids'],
...
for name, slug, desc in services:
    ...
    p = f"""<section class="hero"><div class="wrap"><p class="eyebrow">{name} in Wilmington</p><h1>{name} for Wilmington businesses that need to be found, trusted, and understood by AI search.</h1><p class="lead">{desc} {BRAND} keeps the work practical: stronger pages, clearer offers, local proof, structured FAQs, Google Business Profile alignment, and conversion paths that make the next step obvious.</p><div class="actions"><a class="btn" href="/contact.html">Ask how this works</a><a class="btn ghost" href="tel:{PHONE_E164}">Call {PHONE_DISPLAY}</a></div></div></section><section class="section"><div class="two"><article class="card"><p class="eyebrow">What you get</p><h2>{d['focus']}</h2><ul>{bullet_html}</ul></article><article class="card"><p class="eyebrow">Why this matters</p><h2>How this helps you win more trust</h2><p>{d['value']}</p></article></div></section><section class="section dark"><div><p class="eyebrow">How the work happens</p><h2>Built as part of a connected growth system, not a one-off tactic.</h2><div class="cards">{process_html}</div></div></section><section class="section"><div class="two"><article class="card"><p class="eyebrow">What improves</p><h2>{d['outcome']}</h2><p>The goal is not to make you sound bigger than you are. The goal is to make you easier to find, easier to understand, easier to trust, and easier to contact.</p></article><article class="card"><p class="eyebrow">Ready to improve the lead path?</p><h2>Start with the foundation, then keep improving it.</h2><p>We look at the website, local search presence, Google Business Profile, service pages, and lead path together so each month of work compounds instead of feeling random.</p><a class="btn" href="/contact.html">Talk about the growth plan</a></article></div></section>"""
